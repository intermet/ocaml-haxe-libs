...
let action_id = function
  ...
  | AUnknown (id, _) -> id
  | op -> (
      try Hashtbl.find opcodes op with Not_found -> error "Unknown opcode id")

let action_data_length = function
  | AGotoFrame _ -> 2
  | AGetURL (url, target) -> 2 + String.length url + String.length target
  | ASetReg _ -> 1
  | AStringPool strs ->
  ...
  | AFunction2 f ->
      ...
      List.fold_left
        (fun acc (_, s) -> acc + 2 + String.length s)
        base f.f2_args
  | ASetTarget target -> String.length target + 1
  | AGotoLabel label -> String.length label + 1
  | AWaitForFrame2 _ -> 1
  | ATry t -> (
      1 + 6
      +
      ...
      | TryRegister _ -> 1)
  | AWith _ -> 2 (* the string does not count in length *)
  | APush items ->
  ...
  | AFunction f ->
      List.fold_left
        (fun acc s -> acc + 1 + String.length s)
        4 (f.f_name :: f.f_args)
  | ACondJump _ -> 2
  | AGotoFrame2 (_, id) -> 1 + if id = None then 0 else 2
  | AUnknown (_, data) -> String.length data
  | _ -> 0
...
let actions_length acts =
  DynArray.fold_left
    (fun acc a -> acc + action_length a)
    (action_length AEnd) acts

let read_mm_double ch =
  let i1 = Int64.of_int32 (read_real_i32 ch) in
  let i2 = Int64.of_int32 (read_real_i32 ch) in
  let i2 =
    if i2 < Int64.zero then Int64.add i2 (Int64.shift_left Int64.one 32) else i2
  in
...
let read_string_max ch len =
  ...
  let rec loop l =
    if l = 0 then
      ...
      let c = read ch in
      if c = '\000' then Buffer.contents b
      else (
        Buffer.add_char b c;
        loop (l - 1))
  in
...
let parse_function_decl ch =
  ...
  { f_name = name; f_args = args; f_codelen = clen }
...
let parse_action (ch : IO.input) : Swf.action =
  ...
  let act =
    ...
    | 0x8F ->
        ...
        let tstyle =
          if flags land 4 == 0 then TryVariable (read_string ch)
          else TryRegister (read_byte ch)
        in
        ATry
          {
            tr_style = tstyle;
    ...
    | 0x94 ->
        ...
        AWith size
    | 0x96 -> APush (parse_push_items ch len)
    | 0x99 -> AJump (read_i16 ch)
    | 0x9A -> AGetURL2 (read_byte ch)
    | 0x9B -> AFunction (parse_function_decl ch)
    | 0x9D -> ACondJump (read_i16 ch)
    | 0x9E -> ACallFrame
    | 0x9F ->
        let flags = read_byte ch in
    ...
    | _ -> (
        try Hashtbl.find opcodes_rev id
        with Not_found ->
          printf "Unknown Action 0x%.2X (%d)\n" id len;
          AUnknown (id, nread_string ch len))
  in
  let () =
    printf "\t [action] %s id = %d (0x%.2X)\n" (Swf.show_action act) id id
  in
  let len2 = action_data_length act in
  if len <> len2 then
    error (sprintf "Datalen mismatch for action 0x%.2X (%d != %d)" id len len2);
  act
  ...
  let process_jump curindex = function
    ...
    | ATry t ->
        ...
        let cindex =
          ...
          | None -> None
          | Some size -> Some (size_to_jump_index acts (curindex + tindex) size)
        in
        let findex =
          ...
          | Some size ->
              Some
                (size_to_jump_index acts
                   (curindex + tindex
                   + match cindex with None -> 0 | Some i -> i)
                   size)
        in
        DynArray.set acts curindex
          (ATry
             {
               t with
               tr_trylen = tindex;
               tr_catchlen = cindex;
               tr_finallylen = findex;
             })
    | _ -> ()
  in
...
let write_action_data acts curindex ch = function
  | AGotoFrame frame -> write_ui16 ch frame
  | AGetURL (url, target) ->
      write_string ch url;
      write_string ch target
  | ASetReg reg -> write_byte ch reg
  | AStringPool strs ->
  ...
  | AWaitForFrame (frame, skip) ->
      write_ui16 ch frame;
      write_byte ch skip
  | ASetTarget target -> write_string ch target
  | AGotoLabel label -> write_string ch label
  | AWaitForFrame2 n -> write_byte ch n
  | AFunction2 f ->
  ...
  | ATry t -> (
      ...
      let csize =
        match t.tr_catchlen with
        | None -> 0
        | Some idx -> jump_index_to_size acts (curindex + t.tr_trylen) idx
      in
      let fsize =
        match t.tr_finallylen with
        | None -> 0
        | Some idx ->
            jump_index_to_size acts
              (curindex + t.tr_trylen
              + match t.tr_catchlen with None -> 0 | Some n -> n)
              idx
      in
      let flags =
        (if t.tr_catchlen <> None then 1 else 0)
        lor (if t.tr_finallylen <> None then 2 else 0)
        lor match t.tr_style with TryRegister _ -> 4 | TryVariable _ -> 0
      in
      write_byte ch flags;
      write_ui16 ch tsize;
      write_ui16 ch csize;
  ...
  | ACondJump target ->
      let size = jump_index_to_size acts curindex target in
      write_i16 ch size
  | AGotoFrame2 (play, None) -> write_byte ch (if play then 1 else 0)
  | AGotoFrame2 (play, Some delta) ->
      write_byte ch (if play then 3 else 2);
      write_ui16 ch delta
  | ACallFrame -> ()
  | AUnknown (_, data) -> nwrite_string ch data
  | _ -> assert false
...
let action_string get_ident pos = function
  ...
  | AFunction2 f ->
      ...
      Buffer.add_string b
        (String.concat ","
           (List.map (fun (n, str) -> sprintf "%d:%s" n str) f.f2_args));
      Buffer.add_char b ')';
      Buffer.add_string b
        (sprintf " nregs:%d flags:%d " f.f2_nregs (f2_flags_value f.f2_flags));
      ...
      List.iter
        (fun it ->
          ...
          | PString s ->
              ...
              Buffer.add_string b s;
              Buffer.add_char b '"'
          | PFloat _ -> Buffer.add_string b "<float>"
          | PNull -> Buffer.add_string b "null"
          | PUndefined -> Buffer.add_string b "undefined"
          | PReg n -> Buffer.add_string b (sprintf "reg:%d" n)
          | PBool fl -> Buffer.add_string b (if fl then "true" else "false")
          | PDouble _ -> Buffer.add_string b "<double>"
          | PInt i -> Buffer.add_string b (Int32.to_string i)
          | PStack n | PStack2 n ->
              Buffer.add_char b '[';
  ...
  | op -> ( try Hashtbl.find opcodes_names op with Not_found -> assert false)
