...
let rect_nbits r = r.rect_nbits
let bigrect_nbits r = r.brect_nbits

let rgba_nbits c =
  max (max (_nbits c.r) (_nbits c.g)) (max (_nbits c.b) (_nbits c.a))

let cxa_nbits c = c.cxa_nbits
let matrix_part_nbits m = m.m_nbits
let rgb_length = 3
let rgba_length = 4
...
let matrix_length m =
  ...
  let nbits =
    2
    + opt_len matrix_part_len m.scale
    + opt_len matrix_part_len m.rotate
    + matrix_part_len m.trans
  in
  (nbits + 7) / 8
...
let cxa_length c =
  ...
  let nbits =
    6
    + opt_len (const (nbits * 4)) c.cxa_add
    + opt_len (const (nbits * 4)) c.cxa_mult
  in
  (nbits + 7) / 8

let clip_event_length c =
  (if !swf_version >= 6 then 4 else 2)
  + 4
  + opt_len (const 1) c.cle_key
  + actions_length c.cle_actions

let clip_events_length l =
  (if !swf_version >= 6 then 10 else 6) + sum clip_event_length l

let export_length e = 2 + string_length e.exp_name
let import_length i = 2 + string_length i.imp_name
let sound_length s = 2 + 1 + 4 + String.length s.so_data
...
let shape_fill_style_length s =
  ...
  | SFSLinearGradient (m, g) | SFSRadialGradient (m, g, None) ->
      matrix_length m + gradient_length g
...
let shape_line_style_length s =
  2
  +
  match s.sls_flags with
  | None -> color_length s.sls_color
  | Some _ ->
      2
      + (match s.sls_fill with
        | None -> color_length s.sls_color
        | Some f -> shape_fill_style_length f)
...
let font_shape_records_length records =
  ...
  List.iter
    (fun r ->
      ...
      | SRCurvedEdge s -> nbits := !nbits + (s.scer_nbits * 4)
      | SRStraightEdge s -> (
          nbits :=
            !nbits + 1
            +
            ...
            | Some _, None | None, Some _ -> 1 + s.sser_nbits
...
let shape_length s =
  2 + rect_length s.sh_bounds
  + opt_len (fun (r, _) -> rect_length r + 1) s.sh_bounds2
  + shape_with_style_length s.sh_style

let bitmap_lossless_length b = 2 + 1 + 2 + 2 + String.length b.bll_data

let morph_shape_length s =
  2
  + rect_length s.msh_start_bounds
  + rect_length s.msh_end_bounds
  + String.length s.msh_data

let text_record_length t r =
  1
  + opt_len (const 4) r.txr_font
  + opt_len color_length r.txr_color
  + opt_len (const 2) r.txr_dx
  + opt_len (const 2) r.txr_dy
  + 1
  + ((((t.txt_ngbits + t.txt_nabits) * List.length r.txr_glyphs) + 7) / 8)

let text_length t =
  2
  + big_rect_length t.txt_bounds
  + matrix_length t.txt_matrix + 2
  + sum (text_record_length t) t.txt_records
  + 1
...
let filters_length l =
  ...
  + sum
      (fun f ->
        ...
        +
        match f with
        | FDropShadow s | FBlur s | FGlow s | FBevel s | FAdjustColor s ->
            String.length s
        | FGradientGlow fg | FGradientBevel fg ->
            1
            + ((rgba_length + 1) * List.length fg.fgr_colors)
            + String.length fg.fgr_data)
      l

let button_record_length r =
  1 + 2 + 2 + matrix_length r.btr_mpos
  + (match r.btr_color with None -> 0 | Some c -> cxa_length c)
  + opt_len filters_length r.btr_filters
  + match r.btr_blendmode with None -> 0 | Some c -> 1

let button_action_length r = 2 + 2 + actions_length r.bta_actions

let button2_length b =
  2 + 1 + 2 + 1
  + sum button_record_length b.bt2_records
  + sum button_action_length b.bt2_actions

let cid_data_length c = 2 + String.length c.cd_data
let edit_text_layout_length = 9
let header_length h = 3 + 1 + rect_length h.h_size + 2 + 4
...
let rec tag_data_length = function
  | TEnd -> 0
  | TShowFrame -> 0
  | TShape s -> shape_length s
  | TRemoveObject _ -> 4
  | TBitsJPEG b -> 2 + String.length b.jpg_data
  | TJPEGTables tab -> String.length tab
  | TSetBgColor _ -> rgb_length
  | TFont c -> cid_data_length c
  | TText t -> text_length t
  | TDoAction acts -> actions_length acts
  | TFontInfo c -> cid_data_length c
  | TSound s -> sound_length s
  | TStartSound s -> 2 + String.length s.sts_data
  | TBitsLossless b -> bitmap_lossless_length b
  | TBitsJPEG2 b ->
      2 + opt_len String.length b.bd_table + String.length b.bd_data
  | TShape2 s -> shape_length s
  | TProtect -> 0
  | TPlaceObject2 p -> place_object_length p false
  | TRemoveObject2 _ -> 2
  | TShape3 s -> shape_length s
  | TText2 t -> text_length t
  | TButton2 b -> button2_length b
  | TBitsJPEG3 b ->
      2 + 4
      + opt_len String.length b.bd_table
      + String.length b.bd_data
      + opt_len String.length b.bd_alpha
  | TBitsLossless2 b -> bitmap_lossless_length b
  | TEditText t -> edit_text_length t
  | TClip c -> 4 + sum tag_length (tag_end :: c.c_tags)
  | TProductInfo s -> String.length s
  | TFrameLabel (label, id) -> (
      string_length label + match id with None -> 0 | Some _ -> 1)
  | TSoundStreamHead2 data -> String.length data
  | TMorphShape s | TMorphShape2 s -> morph_shape_length s
  | TFont2 c | TFont3 c | TFontAlignZones c -> cid_data_length c
  | TExport el -> 2 + sum export_length el
  | TImport (url, il) -> string_length url + 2 + sum import_length il
  | TDoInitAction i -> 2 + actions_length i.dia_actions
  | TVideoStream c -> cid_data_length c
  | TVideoFrame c -> cid_data_length c
  | TFontInfo2 c -> cid_data_length c
  | TDebugID s -> String.length s
  | TEnableDebugger2 (_, pass) -> 2 + string_length pass
  | TScriptLimits _ -> 4
  | TFilesAttributes _ -> 4
  | TPlaceObject3 p -> place_object_length p true
  | TImport2 (url, il) -> string_length url + 1 + 1 + 2 + sum import_length il
  | TCSMSettings c -> cid_data_length c
  | TF9Classes l -> 2 + sum (fun c -> string_length c.f9_classname + 2) l
  | TMetaData meta -> string_length meta
  | TScale9 (_, r) -> 2 + rect_length r
  | TActionScript3 (id, a) ->
      (match id with None -> 0 | Some (id, f) -> 4 + string_length f)
      + As3parse.as3_length a
  | TShape4 s -> shape_length s
  | TScenes (sl, fl) ->
      As3parse.int_length (List.length sl)
      + sum (fun (n, s) -> As3parse.int_length n + string_length s) sl
      + As3parse.int_length (List.length fl)
      + sum (fun (n, s) -> As3parse.int_length n + string_length s) fl
  | TBinaryData (_, data) -> 2 + 4 + String.length data
  | TBigBinaryData (_, data) ->
      2 + 4 + List.fold_left (fun acc s -> acc + String.length s) 0 data
  | TFontName c -> cid_data_length c
  | TBitsJPEG4 b ->
      2 + 2 + 4
      + opt_len String.length b.bd_table
      + String.length b.bd_data
      + opt_len String.length b.bd_alpha
  | TFont4 c -> cid_data_length c
...
let skip ch n = seek_in ch (Stdlib.pos_in ch + n)
...
let read_gradient ch is_rgba =
  ...
  if is_rgba then GradientRGBA (read_count n grad_rgba (), flags)
  else GradientRGB (read_count n grad_rgb (), flags)
...
let read_rect ch =
  ...
  { rect_nbits = nbits; left; right; top; bottom }

let rec read_multi_bits b n =
  if n <= 30 then [ read_bits b n ]
  else
...
let read_cxa ch =
  ...
  { cxa_nbits = nbits; cxa_add = add; cxa_mult = mult }

let read_event ch = (if !swf_version >= 6 then read_i32 else read_ui16) ch
...
let rec write_multi_bits b n l =
  if n <= 30 then
    ...
    | [] ->
        write_bits b 30 0;
        write_multi_bits b (n - 30) []
    | x :: l ->
        write_bits b 30 x;
        write_multi_bits b (n - 30) l
...
let write_cxa ch c =
  ...
  | Some c1, Some c2 ->
      write_bits b 2 3;
      write_bits b 4 nbits;
      List.iter (write_bits b ~nbits)
        [ c2.r; c2.g; c2.b; c2.a; c1.r; c1.g; c1.b; c1.a ]);
  flush_bits b
...
let parse_clip_events ch =
  ...
  let rec loop () =
    ...
    if events = 0 then []
      ...
      let e =
        { cle_events = events; cle_key = key; cle_actions = parse_actions ch }
      in
      e :: loop ())
  in
  loop ()
...
let parse_shape_line_style ch vshape =
  ...
  if vshape >= 4 then
		...
		*)
    {
      ...
      sls_fill =
        (if fill then Some (parse_shape_fill_style ch vshape) else None);
  ...
  else
    {
      sls_width = width;
      sls_color =
        (if vshape = 3 then ColorRGBA (read_rgba ch)
        else ColorRGB (read_rgb ch));
      sls_fill = None;
      sls_flags = None;
      sls_miter = None;
    }
...
let parse_shape_curved_edge_record b flags =
  ...
  { scer_nbits = nbits; scer_cx = cx; scer_cy = cy; scer_ax = ax; scer_ay = ay }
...
let parse_shape_straight_edge_record b flags =
  ...
  let l =
    if is_general then
      ...
      if is_vertical then (None, Some p) else (Some p, None)
  in
  { sser_nbits = nbits; sser_line = l }
    ...
    if flags = 0 then []
      ...
      let r =
        if flags land 32 = 0 then
          SRStyleChange
            (parse_shape_style_change_record ch b flags nlbits nfbits vshape)
        else if flags land 48 = 32 then
          SRCurvedEdge (parse_shape_curved_edge_record b flags)
        else SRStraightEdge (parse_shape_straight_edge_record b flags)
      in
      r :: loop ()
  in
  loop ()
...
let parse_shape_with_style ch vshape =
  ...
  let records = parse_shape_records ch nlbits nfbits vshape in
  {
    ...
    sws_records =
      { srs_nlbits = nlbits; srs_nfbits = nfbits; srs_records = records };
  }
...
let parse_shape ch len vshape =
  ...
  { sh_id = id; sh_bounds = bounds; sh_bounds2 = bounds2; sh_style = style }
...
let parse_text ch is_txt2 =
  ...
  let read_glyph bits =
    ...
    { txg_index = indx; txg_advanced = adv }
  in
  let rec loop () =
    ...
    if flags = 0 then []
      ...
      let color =
        if flags land 4 <> 0 then
          Some
            (if is_txt2 then ColorRGBA (read_rgba ch)
            else ColorRGB (read_rgb ch))
        else None
      in
      ...
      let font =
        if flags land 8 <> 0 then Some (font_id, read_ui16 ch) else None
      in
...
let parse_edit_text ch =
  ...
  let layout =
    if flags land (1 lsl 13) <> 0 then Some (parse_edit_text_layout ch)
    else None
  in
...
let parse_morph_shape ch len =
  ...
  let data =
    nread_string ch (len - 2 - rect_length sbounds - rect_length ebounds)
  in
  {
    msh_id = id;
...
let parse_filter_gradient ch =
  ...
  { fgr_colors = List.combine colors cvals; fgr_data = data }
...
let rec parse_button_actions ch =
  ...
  let bta = { bta_flags = flags; bta_actions = actions } in
  if size = 0 then [ bta ] else bta :: parse_button_actions ch
...
let parse_import ch =
  ...
  { imp_id = cid; imp_name = name }

let rec parse_tag ch h pos : Swf.tag =
  let id = h lsr 6 in
  ...
  let t =
    ...
    | 0x00 -> TEnd
    | 0x01 -> TShowFrame
    | 0x02 when !full_parsing -> TShape (parse_shape ch len 1)
    (* 0x03 invalid *)
    (*//0x04 TPlaceObject *)
    | 0x05 ->
        let cid = read_ui16 ch in
        let depth = read_ui16 ch in
        TRemoveObject { rmo_id = cid; rmo_depth = depth }
    | 0x06 ->
        let id = read_ui16 ch in
        let data = nread_string ch (len - 2) in
        TBitsJPEG { jpg_id = id; jpg_data = data }
    (*//0x07 TButton *)
    | 0x08 -> TJPEGTables (nread_string ch len)
    | 0x09 -> TSetBgColor (read_rgb ch)
    | 0x0A -> TFont (parse_cid_data ch len)
    | 0x0B when !full_parsing -> TText (parse_text ch false)
    | 0x0C -> TDoAction (parse_actions ch)
    | 0x0D -> TFontInfo (parse_cid_data ch len)
    | 0x0E ->
    ...
    | 0x0F ->
        let sid = read_ui16 ch in
        let data = nread_string ch (len - 2) in
        TStartSound { sts_id = sid; sts_data = data }
    (* 0x10 invalid *)
    (*//0x11 TButtonSound *)
    (*//0x12 TSoundStreamHead *)
    (*//0x13 TSoundStreamBlock *)
    | 0x14 -> TBitsLossless (parse_bitmap_lossless ch len)
    | 0x15 ->
        let id = read_ui16 ch in
        let data = nread_string ch (len - 2) in
    ...
    | 0x16 when !full_parsing -> TShape2 (parse_shape ch len 2)
    (*//0x17 TButtonCXForm *)
    | 0x18 -> TProtect
    (* 0x19 invalid *)
    | 0x1A when !full_parsing -> TPlaceObject2 (parse_place_object ch false)
    (* 0x1B invalid *)
    | 0x1C ->
        let depth = read_ui16 ch in
        TRemoveObject2 depth
    (* 0x1D-1F invalid *)
    | 0x20 when !full_parsing -> TShape3 (parse_shape ch len 3)
    | 0x21 when !full_parsing -> TText2 (parse_text ch true)
    | 0x22 when !full_parsing -> TButton2 (parse_button2 ch len)
    | 0x23 ->
        let id = read_ui16 ch in
    ...
    | 0x24 -> TBitsLossless2 (parse_bitmap_lossless ch len)
    | 0x25 when !full_parsing -> TEditText (parse_edit_text ch)
    (* 0x26 invalid *)
    | 0x27 ->
        let cid = read_ui16 ch in
        let fcount = read_ui16 ch in
        let tags = parse_tag_list ch pos in
        TClip { c_id = cid; c_frame_count = fcount; c_tags = tags }
    (* 0x28 invalid *)
    | 0x29 ->
        (* undocumented ? *)
    ...
    | 0x2B ->
        ...
        let id =
          if len = String.length label + 2 then Some (read ch) else None
        in
        TFrameLabel (label, id)
    (* 0x2C invalid *)
    | 0x2D -> TSoundStreamHead2 (nread_string ch len)
    | 0x2E when !full_parsing -> TMorphShape (parse_morph_shape ch len)
    (* 0x2F invalid *)
    | 0x30 when !full_parsing -> TFont2 (parse_cid_data ch len)
    (* 0x31-37 invalid *)
    | 0x38 ->
        let read_export () =
          let cid = read_ui16 ch in
          let name = read_string ch in
          { exp_id = cid; exp_name = name }
        in
    ...
    | 0x3B ->
        let cid = read_ui16 ch in
        let actions = parse_actions ch in
        TDoInitAction { dia_id = cid; dia_actions = actions }
    | 0x3C -> TVideoStream (parse_cid_data ch len)
    | 0x3D -> TVideoFrame (parse_cid_data ch len)
    | 0x3E -> TFontInfo2 (parse_cid_data ch len)
    | 0x3F ->
        (* undocumented ? *)
    ...
    | 0x45 ->
        ...
        if flags lor mask <> mask then
          failwith ("Invalid file attributes " ^ string_of_int flags);
        TFilesAttributes
          {
    ...
    | 0x46 when !full_parsing -> TPlaceObject3 (parse_place_object ch true)
    | 0x47 ->
        let url = read_string ch in
    ...
    | 0x49 when !full_parsing -> TFontAlignZones (parse_cid_data ch len)
    | 0x4A -> TCSMSettings (parse_cid_data ch len)
    | 0x4B when !full_parsing -> TFont3 (parse_cid_data ch len)
    | 0x4C ->
        ...
        let rec loop i =
          if i = 0 then []
            ...
            { f9_cid = (if a = 0 then None else Some a); f9_classname = s }
            :: loop (i - 1)
        in
        TF9Classes (loop i)
    | 0x4D -> TMetaData (read_string ch)
    | 0x4E ->
        let cid = read_ui16 ch in
    ...
    | 0x53 when !full_parsing -> TShape4 (parse_shape ch len 4)
    | 0x54 when !full_parsing -> TMorphShape2 (parse_morph_shape ch len)
    (* 0x55 invalid *)
    | 0x56 ->
        let scenes =
          read_count (As3parse.read_int ch)
            (fun () ->
    ...
    | 0x5B -> TFont4 (parse_cid_data ch len)
    | _ ->
  ...
  *)
  let tag = { tid = gen_id (); tdata = t; textended = extended } in
  tag

and parse_tag_list ch pos =
  (* let () = Printf.printf "[parse] parse tag list\n" in *)
  let rec loop acc =
    let pos1 = pos () in
    let h = try read_ui16 ch with IO.No_more_input -> 0 in
    match parse_tag ch h pos with
    | { tdata = TEnd } -> List.rev acc
    | t ->
        let pos2 = pos () in
        let () =
          Printf.printf "[tag] %s id = %d bytes : %d - %d (%d)\n"
            (Swf.tag_name t.tdata) (tag_id t.tdata) pos1 pos2 (pos2 - pos1)
        in
        loop (t :: acc)
  in
  loop []
...
let parse ch =
  ...
  (* file length *)
  let compressed, ch =
    if sign = "CWS" then (true, inflate ch) else (false, ch)
  in
  let ch, _pos = IO.pos_in ch in
  let pos () = 8 + _pos () in
  ...
  in
  let data = (h, parse_tag_list ch pos) in
...
let write_clip_events ch event_list =
  write_ui16 ch 0;
  let all_events =
    List.fold_left (fun acc c -> acc lor c.cle_events) 0 event_list
  in
...
let write_shape_fill_style ch s =
  ...
  | SFSBitmap b ->
      write_byte ch
        (match (b.sfb_repeat, b.sfb_smooth) with
        | true, false -> 0x40
        | false, false -> 0x41
        | true, true -> 0x42
        | false, true -> 0x43);
...
let write_shape_line_style ch l =
  ...
  | Some fill -> write_shape_fill_style ch fill
...
let write_shape_record ch b nlbits nfbits = function
  | SRStyleChange s -> write_shape_style_change_record ch b nlbits nfbits s
  | SRCurvedEdge s ->
      write_bits b 2 2;
...
let write_button_record ch r =
  ...
  (match r.btr_color with None -> () | Some c -> write_cxa ch c);
  opt
    (fun l ->
      write_byte ch (List.length l);
      List.iter (write_filter ch) l)
    r.btr_filters;
  match r.btr_blendmode with None -> () | Some c -> write_byte ch c
...
let write_button2 ch b =
  ...
  if b.bt2_actions <> [] then
    write_ui16 ch (3 + sum button_record_length b.bt2_records)
  else write_ui16 ch 0;
...
let write_place_object ch p v3 =
  ...
  if v3 then
    write_byte ch
      (make_flags [ flag p.po_filters; flag p.po_blend; flag p.po_bcache ]);
...
let rec write_tag_data ch = function
  ...
  | TJPEGTables tab -> nwrite_string ch tab
  | TSetBgColor c -> write_rgb ch c
  | TFont c -> write_cid_data ch c
  | TText t -> write_text ch t
  | TDoAction acts -> write_actions ch acts
  | TFontInfo c -> write_cid_data ch c
  | TSound s ->
  ...
  | TBitsLossless b -> write_bitmap_lossless ch b
  | TBitsJPEG2 b ->
      write_ui16 ch b.bd_id;
      opt (nwrite_string ch) b.bd_table;
      nwrite_string ch b.bd_data
  | TShape2 s -> write_shape ch s
  | TProtect -> ()
  | TPlaceObject2 p -> write_place_object ch p false
  | TRemoveObject2 depth -> write_ui16 ch depth
  | TShape3 s -> write_shape ch s
  | TText2 t -> write_text ch t
  | TButton2 b -> write_button2 ch b
  | TBitsJPEG3 b ->
      write_ui16 ch b.bd_id;
  ...
  | TBitsLossless2 b -> write_bitmap_lossless ch b
  | TEditText t -> write_edit_text ch t
  | TClip c ->
      write_ui16 ch c.c_id;
      write_ui16 ch c.c_frame_count;
      List.iter (write_tag ch) c.c_tags;
      write_tag ch tag_end
  | TProductInfo s -> nwrite_string ch s
  | TFrameLabel (label, id) ->
      write_string ch label;
      opt (write ch) id
  | TSoundStreamHead2 data -> nwrite_string ch data
  | TMorphShape s -> write_morph_shape ch s
  | TFont2 c -> write_cid_data ch c
  | TExport el ->
  ...
  | TVideoStream c -> write_cid_data ch c
  | TVideoFrame c -> write_cid_data ch c
  | TFontInfo2 c -> write_cid_data ch c
  | TDebugID s -> nwrite_string ch s
  ...
  | TFilesAttributes f ->
      ...
      in
      write_i32 ch flags
  | TPlaceObject3 p -> write_place_object ch p true
  | TImport2 (url, il) ->
      write_string ch url;
      write_byte ch 1;
      write_byte ch 0;
      ...
      List.iter
        (fun i ->
          ...
          write_string ch i.imp_name)
        il
  | TFontAlignZones c -> write_cid_data ch c
  | TCSMSettings c -> write_cid_data ch c
  | TFont3 c -> write_cid_data ch c
  | TF9Classes l ->
      ...
      List.iter
        ...
        l
  | TMetaData meta -> write_string ch meta
  | TScale9 (cid, r) ->
      write_ui16 ch cid;
      write_rect ch r
  | TActionScript3 (id, a) ->
      ...
      As3parse.write ch a
  | TShape4 s -> write_shape ch s
  | TMorphShape2 m -> write_morph_shape ch m
  | TScenes (sl, fl) ->
  ...
  | TFontName c -> write_cid_data ch c
  | TBitsJPEG4 b ->
      write_ui16 ch b.bd_id;
      write_i32 ch (String.length b.bd_data + opt_len String.length b.bd_table);
      ...
      nwrite_string ch b.bd_data;
      opt (nwrite_string ch) b.bd_alpha
  | TFont4 c -> write_cid_data ch c
  | TUnknown (_, data) -> nwrite_string ch data

and write_tag ch t =
  let id = tag_id t.tdata in
  let dlen = tag_data_length t.tdata in
  if t.textended || dlen >= 63 then (
    write_ui16 ch ((id lsl 6) lor 63);
    write_i32 ch dlen)
  else write_ui16 ch ((id lsl 6) lor dlen);
...
let scan fid f t =
  match t.tdata with
  | TEnd | TShowFrame | TJPEGTables _ | TSetBgColor _ | TDoAction _
  | TActionScript3 _ | TProtect | TRemoveObject2 _ | TFrameLabel _
  | TSoundStreamHead2 _ | TScenes _ | TEnableDebugger2 _ | TMetaData _
  | TScriptLimits _ | TDebugID _ | TFilesAttributes _ | TProductInfo _ ->
      ()
  | TF9Classes l ->
      List.iter
        (fun c ->
          match c.f9_cid with None -> () | Some id -> c.f9_cid <- Some (f id))
        l
  | TShape s | TShape2 s | TShape3 s | TShape4 s ->
      s.sh_id <- fid s.sh_id;
      let loop fs =
        List.iter
          (fun s ->
      ...
      List.iter
        (fun s ->
          match s with
          | SRStyleChange { scsr_new_styles = Some s } -> loop s.sns_fill_styles
          | _ -> ())
        s.sh_style.sws_records.srs_records
  | TRemoveObject r -> r.rmo_id <- f r.rmo_id
  | TBitsJPEG b -> b.jpg_id <- fid b.jpg_id
  | TBitsJPEG2 b -> b.bd_id <- fid b.bd_id
  | TText t | TText2 t ->
      t.txt_id <- fid t.txt_id;
      List.iter
        (fun r ->
          match r.txr_font with
          | None -> ()
          | Some (id, id2) -> r.txr_font <- Some (f id, id2))
        t.txt_records
  | TEditText t -> (
      t.edt_id <- fid t.edt_id;
      match t.edt_font with
      | None -> ()
      | Some (id, h) -> t.edt_font <- Some (f id, h))
  | TSound s -> s.so_id <- fid s.so_id
  | TStartSound s -> s.sts_id <- f s.sts_id
  | TBitsLossless b | TBitsLossless2 b -> b.bll_id <- fid b.bll_id
  | TPlaceObject2 p ->
      p.po_cid <- (match p.po_cid with None -> None | Some id -> Some (f id))
  | TButton2 b ->
      b.bt2_id <- fid b.bt2_id;
      List.iter (fun r -> r.btr_cid <- f r.btr_cid) b.bt2_records
  | TBitsJPEG3 j -> j.bd_id <- fid j.bd_id
  | TClip c -> c.c_id <- fid c.c_id
  | TMorphShape s | TMorphShape2 s -> s.msh_id <- fid s.msh_id
  | TFont c | TFont2 c | TFont3 c | TFont4 c -> c.cd_id <- fid c.cd_id
  | TExport el -> List.iter (fun e -> e.exp_id <- f e.exp_id) el
  | TImport (_, il) | TImport2 (_, il) ->
      List.iter (fun i -> i.imp_id <- fid i.imp_id) il
  | TDoInitAction a -> a.dia_id <- f a.dia_id
  | TVideoStream c -> c.cd_id <- fid c.cd_id
  | TVideoFrame c -> c.cd_id <- f c.cd_id
  | TPlaceObject3 p ->
      p.po_cid <- (match p.po_cid with None -> None | Some id -> Some (f id))
  | TCSMSettings c -> c.cd_id <- f c.cd_id
  | TBinaryData (id, data) -> t.tdata <- TBinaryData (fid id, data)
  | TBigBinaryData (id, data) -> t.tdata <- TBigBinaryData (fid id, data)
  | TFontAlignZones c | TFontInfo c | TFontInfo2 c | TFontName c ->
      c.cd_id <- f c.cd_id
  | TScale9 (id, r) -> t.tdata <- TScale9 (f id, r)
  | TBitsJPEG4 j -> j.bd_id <- fid j.bd_id
  | TUnknown _ -> ()
