 ...
 *)
type float16 = int [@@deriving show]
type unknown = string [@@deriving show]
type binary = string [@@deriving show]
type action_count = int [@@deriving show]
type rgb = { cr : int; cg : int; cb : int } [@@deriving show]
type rgba = { r : int; g : int; b : int; a : int } [@@deriving show]
type color = ColorRGB of rgb | ColorRGBA of rgba [@@deriving show]
...
type matrix_part = { m_nbits : int; mx : int; my : int } [@@deriving show]
...
type try_style = TryRegister of int | TryVariable of string [@@deriving show]
...
type export = { mutable exp_id : int; exp_name : string } [@@deriving show]
type import = { mutable imp_id : int; imp_name : string } [@@deriving show]

type do_init_action = { mutable dia_id : int; dia_actions : (actions[@opaque]) }
[@@deriving show]
...
type start_sound = { mutable sts_id : int; sts_data : unknown }
[@@deriving show]
...
type filter_gradient = { fgr_colors : (rgba * int) list; fgr_data : unknown }
[@@deriving show]
...
type bitmap_jpg = { mutable jpg_id : int; jpg_data : binary } [@@deriving show]
...
type cid_data = { mutable cd_id : int; cd_data : binary } [@@deriving show]
type text_glyph = { txg_index : int; txg_advanced : int } [@@deriving show]
...
type button_action = { bta_flags : int; bta_actions : (actions[@opaque]) }
[@@deriving show]
...
type remove_object = { mutable rmo_id : int; rmo_depth : int } [@@deriving show]
...
type f9class = { mutable f9_cid : int option; f9_classname : string }
[@@deriving show]
...
type tag_data =
  ...
  | TMetaData of string
  | TScale9 of int * rect
  | TActionScript3 of (int * string) option * (As3.as3_tag[@opaque])
  | TShape4 of shape
...
and clip = { mutable c_id : int; c_frame_count : int; c_tags : tag list }
...
type font_glyph_data = { font_char_code : int; font_shape : shape_records }
type font_layout_glyph_data = { font_advance : int; font_bounds : rect }
...
let parse (ch : IO.input) : swf = !__parser ch
let write (ch : 'a IO.output) (data : swf) = !__printer (Obj.magic ch) data

let deflate (ch : 'a IO.output) : 'a IO.output =
  Obj.magic (!__deflate (Obj.magic ch) : unit IO.output)

let inflate (ch : IO.input) : IO.input = !__inflate ch

let tag_name (tag : tag_data) =
  match tag with
  | TEnd -> "End"
