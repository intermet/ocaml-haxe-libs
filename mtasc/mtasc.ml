...
type p_style = StyleJava | StyleMSVC
  ...
  let rec loop = function
    ...
    | x :: l -> (
        try
          ...
          let p2 = loop l in
          if p2 = -1 || p2 > p then p else p2
        with Not_found -> loop l)
  in
  let p = loop l in
  if p = -1 then [ str ]
  else if p = 0 then split l (String.sub str 1 (String.length str - 1))
  else
...
let class_name file =
  ...
  (path, try Filename.chop_extension file with _ -> file)

let normalize_path p =
  let l = String.length p in
  if l = 0 then "./" else match p.[l - 1] with '\\' | '/' -> p | _ -> p ^ "/"

let rec parse_class_path base_path path =
  if path = "" then []
  else
    let relative fp l =
      if String.length fp >= 2 && (fp.[0] = '/' || fp.[0] = '.' || fp.[1] = ':')
      then fp :: l
      else fp :: (base_path ^ fp) :: l
    in
    try
    ...
    with Not_found -> relative (normalize_path path) []
...
let read_package path =
  ...
  let rec loop = function
    ...
    | cpath :: l -> (
        ...
        match
          Array.to_list (try Sys.readdir filepath with Sys_error _ -> [||])
        with
        | [] -> loop l
        | files -> (
            match
              List.filter
                (fun f ->
                  String.ends_with (String.lowercase_ascii f) ~suffix:".as")
                files
            with
            | [] -> loop l
