...
type local = { lt : type_decl; lf : int }
...
let rec s_type_decl = function
  ...
  | Function (args, r) ->
      "function ("
      ^ String.concat ", " (List.map s_type_decl args)
      ^ ") : " ^ s_type_decl r
...
let load_class_ref =
  ref (fun _ -> assert false : context -> type_path -> pos -> class_context)
...
let rec is_super sup c =
  if c == sup then true
  else if c.super == c then false
  else is_super sup c.super

let is_number ctx = function
  | Class c
    when c == match ctx.inumber with Class c2 -> c2 | _ -> assert false ->
      true
  | _ -> false

let is_boolean ctx = function
  | Class c when c == match ctx.ibool with Class c2 -> c2 | _ -> assert false ->
      true
  | _ -> false

let is_string ctx = function
  | Class c
    when c == match ctx.istring with Class c2 -> c2 | _ -> assert false ->
      true
  | _ -> false
      ...
      let rec loop = function
        | [] -> (
            try
              ...
              imp.imp_used <- true;
              cl
            with Not_found -> !load_class_ref ctx p pos)
        | imp :: l -> (
            try
              let cl = !load_class_ref ctx (imp.wimp_path, n) pos in
              imp.wimp_used <- true;
              cl
            with Error (Class_not_found p, _) when p = (imp.wimp_path, n) ->
              loop l)
      in
...
let rec is_function cl =
  match cl.path with
  | [], "Function" -> true
  | _ -> if cl.super == cl then false else is_function cl.super
...
let rec unify ta tb p =
  ...
  | Class cl1, Class cl2 -> (
      let rec loop cl1 =
        if cl1 == cl2 || List.exists loop cl1.implements then true
        else if cl1.super == cl1 then false
        else loop cl1.super
      in
  ...
  | (Function _, Class c | Static _, Class c) when c.super == c ->
      () (* unify with Object *)
  | Static _, Class cl
  | Class cl, Static _
  | Class cl, Function _
  | Function _, Class cl
    when is_function cl ->
      ()
  | _, _ -> error (Cannot_unify (ta, tb)) p
...
let rec tcommon ctx ta tb p =
  match (ta, tb) with
  | Void, Void -> Void
  | Void, _ | _, Void -> error (Cannot_unify (ta, tb)) p
  | Dyn, _ | _, Dyn -> Dyn
  | Function _, _ | Static _, _ ->
      tcommon ctx (Class (!load_class_ref ctx ([], "Function") null_pos)) tb p
  | _, Function _ | _, Static _ ->
      tcommon ctx ta (Class (!load_class_ref ctx ([], "Function") null_pos)) p
  | Package _, _ | _, Package _ -> assert false
  | Class a, Class b -> (
      let rec is_sub cl1 cl2 =
        if cl1 == cl2 || List.exists (is_sub cl1) cl2.implements then true
        else if cl2.super == cl2 then false
        else is_sub cl1 cl2.super
      in
      let rec parent cl1 cl2 =
        if is_sub cl2 cl1 then Some cl2
        else
          let rec loop = function
            | [] -> if cl2.super == cl2 then None else parent cl1 cl2.super
            | i :: l -> (
                match parent cl1 i with Some i -> Some i | None -> loop l)
          in
          loop cl2.implements
      in
...
let t_opt ctx p = function
  | None ->
      if !strict_mode && not ctx.current.native then
        error (Custom "Type required in strict mode") p;
      Dyn
  | Some ([], "Void") -> Void
...
let rec has_return any (e, p) =
  ...
  match e with
  | EVars _ | EFunction _ | EBreak | EContinue | EVal _ -> false
  | EReturn None -> any
  | EBlock el -> List.exists has_return el
  | EFor (el, _, _, e) -> List.exists has_return (e :: el)
  | EForIn (e1, _, e2) -> has_return e1 || has_return e2
  | EIf (_, e, eo) -> (
      has_return e || match eo with None -> false | Some e -> has_return e)
  | EWhile (_, e, _) -> has_return e
  | ESwitch (_, cases) -> List.exists (fun (_, e) -> has_return e) cases
  | ETry (e, cl, fo) -> (
      has_return e
      || List.exists (fun (_, _, e) -> has_return e) !cl
      || match fo with None -> false | Some e -> has_return e)
  | EWith (_, e) -> has_return e
  | EReturn (Some _) -> true
...
let rec add_class_field ctx clctx fname stat pub get ft p =
  if pub = IsPrivate && clctx.interface then
    error (Custom "Private fields are not possible in interfaces") p;
  if stat = IsStatic && clctx.interface then
    error (Custom "Static fields are not possible in interfaces") p;
  let h =
    match stat with IsStatic -> clctx.statics | IsMember -> clctx.fields
  in
  let f = try Some (Hashtbl.find h fname) with Not_found -> None in
  match get with
  | Getter | Setter ->
      add_class_field ctx clctx
        ((if get = Getter then "__get__" else "__set__") ^ fname)
        stat pub Normal ft p;
      let t =
        if get = Getter then
          ...
          match ft with
          | Function ([ arg ], r) ->
              if r <> Void && r <> Dyn then
                error (Custom "Setter should not return any value") p;
              arg
          | Function _ -> error (Custom "Setter can only have one parameter") p
          | _ -> assert false
      in
      let f =
        match f with
        ...
        | Some f ->
            {
              ...
              f_type =
                (try
                   ...
                   unify t f.f_type p;
                   t
                 with Error (Cannot_unify _, _) when !use_components ->
                   f.f_type);
              f_static = stat;
              f_public =
                (if pub <> f.f_public then
                 error
                   (Custom
                      "Getter and setter have different public/private \
                       visibility") p
                else pub);
              f_pos = p;
            }
  ...
  | Normal ->
      if
        ...
        || Hashtbl.mem
             (match stat with
             | IsStatic -> clctx.fields
             | IsMember -> clctx.statics)
             fname
      then error (Custom ("Field redefiniton : " ^ fname)) p;
      Hashtbl.add h fname
        {
          f_name = fname;
          f_type = ft;
...
let add_finalizer ctx f = ctx.finalizers := f :: !(ctx.finalizers)
let no_void t p = if t = Void then error (Custom "Void where Object expected") p

let define_local ctx name t p =
  if Hashtbl.mem ctx.locals name then
    error (Custom ("Local variable redefinition : " ^ name)) p;
...
let clean_frame ctx f =
  ctx.frame <- f;
  Hashtbl.iter
    (fun n l -> if l.lf > f then Hashtbl.remove ctx.locals n)
    ctx.locals

let rec resolve t fname =
  match t with
  | Void | Dyn | Function _ -> None
  | Package p ->
      Some
        {
          f_name = fname;
  ...
  | Static c -> (
      try Some (Hashtbl.find c.statics fname)
      with Not_found ->
        if c.super == c then None else resolve (Static c.super) fname)
  | Class c -> (
      try Some (Hashtbl.find c.fields fname)
      with Not_found ->
        if c.super == c then None else resolve (Class c.super) fname)
...
and type_ident ctx name e p =
  ...
  try
    ...
    | Some t -> (
        match t with
        | Void | Static _ | Package _ -> assert false
        | Dyn ->
            set_eval e (EStatic ([ "__With" ], name));
            Dyn
        | Function _ ->
            set_eval e (EStatic ([ "__With" ], name));
            Dyn
        | Class c -> (
            ...
            | None -> raise Not_found
            | Some { f_public = IsPrivate } when not (is_super c ctx.current) ->
                error (Custom "Cannot access private field") p
            | Some f ->
                set_eval e (EStatic ([ "__With" ], name));
                f.f_type))
  with Not_found -> (
    ...
    with Not_found -> (
      ...
      try
        if name = snd ctx.current.path then (
          set_eval e (EStatic ctx.current.path);
          Static ctx.current)
        else
          let f =
            match resolve (Class ctx.current) name with
            | None -> raise Not_found
            | Some f -> f
          in
          if ctx.in_static then
            error
              (Custom
                 ("Cannot access member variable " ^ name
                ^ " in static function"))
              p;
      ...
      with Not_found -> (
        ...
        let rec loop c =
          ...
          with Not_found -> if c.super == c then None else loop c.super
        in
        ...
        | None -> (
            match
              resolve
                (Static (!load_class_ref ctx ([], "TopLevel") null_pos))
                name
            with
            | Some f ->
                if f.f_public = IsPublic then
                  set_eval e (EField ((EConst (Ident "_global"), p), name));
                f.f_type
            | None ->
                if
                  String.length name > 6
                  && String.sub name 0 6 = "_level"
                  &&
                  try
                    int_of_string (String.sub name 6 (String.length name - 6))
                    >= 0
                  with _ -> false
                then Class (!load_class_ref ctx ([], "MovieClip") null_pos)
                else Package [ name ]))))
...
let type_constant ctx c e p =
  ...
  | Ident "this" ->
      if ctx.in_lambda <> None then Dyn
      else (
        if ctx.in_static then
          error (Custom "Cannot access this in static function") p;
        Class ctx.current)
  | Ident "super" ->
      if ctx.in_lambda <> None then Dyn
      else (
        if ctx.in_static then
          error (Custom "Cannot access super in static function") p;
        Class ctx.current.super)
  | Ident name -> type_ident ctx name e p
...
let rec resolve_package ctx v (p : string list) pos =
  ...
  | cname :: fields ->
        ...
        let rec loop acc = function
          ...
          | x :: l -> (
              ...
              try
                ...
                set_eval v vv;
                (Static cl, acc)
              with Error (Class_not_found p, _) when p = cpath ->
                loop (x :: acc) l)
        in
        let t, fields = loop [] (List.rev p) in
        let rec loop t = function
          | [] -> t
          | f :: l -> loop (type_field ctx t f pos) l
        in
        loop t fields
      in
      let rec loop = function
        | [] -> (
            ...
            in
            match last p with
            | x when String.length x > 0 && x.[0] >= 'A' && x.[0] <= 'Z' ->
      ...
      let p2, n, use =
        ...
        with Not_found -> ([], cname, fun () -> ())
      in
      loop
        ((p2 @ (n :: fields), use)
        :: List.map
             (fun imp ->
               ( imp.wimp_path @ (cname :: fields),
                 fun () -> imp.wimp_used <- true ))
             ctx.current.imports.wildcards)

and type_field ctx t f p =
  match resolve t f with
  | None ->
      if not (is_dynamic t) then
        error
          (Custom
             (s_type_decl (match t with Static c -> Class c | _ -> t)
             ^ " has no "
             ^ (match t with Static _ -> "static " | _ -> "")
             ^ "field " ^ f))
          p;
      Dyn
  | Some f ->
      (if f.f_public = IsPrivate then
       match t with
       | Class c | Static c ->
           if not (is_super c ctx.current) then
             if
               match ctx.in_lambda with
               | None -> true
...
let rec type_binop ctx op v1 v2 p =
  ...
  let rec loop = function
    | OpAdd ->
        if t1 == Dyn || t2 == Dyn then Dyn
        else if is_number ctx t1 && is_number ctx t2 then ctx.inumber
        else ctx.istring
    | OpAnd | OpOr | OpXor | OpShl | OpShr | OpUShr | OpMod | OpMult | OpDiv
    | OpSub ->
    ...
    | OpAssign ->
        ...
        t1
    | OpEq | OpPhysEq | OpPhysNotEq | OpNotEq | OpGt | OpGte | OpLt | OpLte ->
        ctx.ibool
    | OpBoolAnd | OpBoolOr -> tcommon ctx t1 t2 p
    | OpAssignOp op ->
        let t = loop op in
        unify t t1 p;
        t1
...
and type_val ?(in_field = false) ctx ((v, p) as e) =
  ...
  | EArray (v1, v2) -> (
      let t = type_val ctx v1 in
      ignore (type_val ctx v2);
      match t with Class { param = Some c } -> Class c | _ -> Dyn)
  | EBinop (op, v1, v2) -> type_binop ctx op v1 v2 p
  | EField (v, f) ->
      ...
      | EField (((EStatic p, pos) as v), _), _
        when f <> "prototype" && fst p <> [ "__With" ] ->
          let rec loop cl =
            if Hashtbl.mem cl.statics f then cl.path
            else if cl.super == cl then p
            else loop cl.super
          in
  ...
  | ECall ((EConst (Ident "super"), _), args) ->
      if not ctx.in_constructor then
        error
          (Custom "Super constructor can only be called in class constructor") p;
      let args = List.map (type_val ctx) args in
      (match ctx.current.super.constructor with
      | None -> ()
      | Some t -> unify (Function (args, Void)) t.f_type p);
      Void
  | ECall (v, args) -> (
      ...
      | Function (fargs, ret) ->
          let rec loop l1 l2 =
            match (l1, l2) with
            | [], _ -> ()
            | l, [] -> List.iter (fun v -> ignore (type_val ctx v)) l
            | v :: l1, a :: l2 ->
      ...
      | _ ->
          error
            (Custom ("Cannot call non-function object " ^ s_type_decl t))
            (pos v))
  | EQuestion (v, v1, v2) ->
  ...
  | ENew (v, vl) -> (
      ...
      | Static cl ->
          ...
          | None -> ()
          | Some t ->
              if t.f_public = IsPrivate && not (is_super cl ctx.current) then
                error (Custom "Cannot call private constructor") p;
              unify (Function (args, Dyn)) t.f_type p);
          Class cl
      | Dyn -> Dyn
      | Class cl when is_function cl -> Dyn
      | t ->
          error (Custom ("Invalid type : " ^ s_type_decl t ^ " for new call")) p
      )
  | ELambda f -> !type_function_ref ~lambda:true ctx (t_object ctx) f p
...
let rec type_expr ctx (e, p) =
  ...
  | EVars (_, _, vl) ->
      List.iter
        (fun (name, tt, v) ->
          let t =
            if !local_inference && v <> None && tt = None then Dyn
            else t_opt ctx p tt
          in
          let t =
            match v with
  ...
  | EFor (inits, conds, incrs, e) ->
      ...
      List.iter (fun v -> no_void (type_val ctx v) (pos v)) conds;
  ...
  | EForIn (decl, v, e) ->
      ...
      | _ -> error (Custom "Invalid forin parameter") p);
      no_void (type_val ctx v) (pos v);
      type_expr ctx e;
      clean_frame ctx f
  | EIf (v, e, eo) -> (
  ...
  | ESwitch (v, cases) ->
      ...
      List.iter
        (fun (v, e) ->
          (match v with
          | None -> ()
          | Some v -> unify (type_val ctx v) t (pos v));
          type_expr ctx e)
        cases
  | ETry (etry, cl, fo) -> (
      type_expr ctx etry;
      ...
      cl :=
        List.map
          (fun (name, t, e) ->
            if !no_type then
              error (Custom "Misplaced catch will fail to catch any exception")
                (pos e);
            let t2 =
              match t with
              | None ->
                  no_type := true;
                  None
              | Some c -> Some (resolve_path ctx c p).path
            in
            let f = new_frame ctx in
  ...
  | EWith (v, e) ->
      ...
      let t = type_val ctx v in
      (match t with
      | Void | Static _ -> error (Custom "Invalid type for 'with' argument") p
      | Package _ -> assert false
      | Dyn | Function _ | Class _ -> ());
      ctx.curwith <- Some t;
      ignore (type_expr ctx e);
      ctx.curwith <- old_with
  | EReturn None ->
      if ctx.returns <> Void && ctx.returns <> Dyn then
        error (Custom "Return type cannot be Void") p
  | EReturn (Some v) -> unify (type_val ctx v) ctx.returns (pos v)
  | EBreak | EContinue -> ()
  | EVal v -> ignore (type_val ctx v)
...
let type_function ?(lambda = false) ctx clctx f p =
  ...
  | Some e ->
      if not lambda then
        verbose_msg ("Typing " ^ s_type_path clctx.path ^ "." ^ f.fname);
      let ctx =
        {
          ctx with
          current =
            (if lambda then
             { clctx with imports = ctx.current.imports; native = false }
            else clctx);
          ...
          in_lambda =
            (if lambda then
             match ctx.in_lambda with
             | None -> Some ctx.current
             | Some _ -> ctx.in_lambda
            else None);
          curwith = None;
        }
      in
...
let rec type_class_fields ctx clctx comp (e, p) =
  ...
  | EVars (stat, pub, vl) ->
      if clctx.interface then
        error (Custom "Interface cannot contain variable declaration") p;
      List.iter
        (fun (vname, vtype, vinit) ->
  ...
  | EFunction f ->
      let t =
        Function
          (List.map (fun (_, t) -> t_opt ctx p t) f.fargs, ret_opt ctx p f)
      in
      if f.fname = snd clctx.path then (
        if f.ftype <> None then
          error (Custom "Constructor return type should not be specified") p;
        if clctx.interface then
          error (Custom "Interface can't have a constructor") p;
      ...
      else add_class_field ctx clctx f.fname f.fstatic f.fpublic f.fgetter t p;
      if f.fexpr <> None && not comp then
        add_finalizer ctx (fun () -> ignore (type_function ctx clctx f p))
  | _ -> assert false
...
let type_class ctx cpath herits e imports file interf native s =
  ...
  in
  Hashtbl.add imports.paths clctx.name
    { imp_path = clctx.path; imp_used = true; imp_pos = pos s };
  if Hashtbl.mem ctx.classes cpath then
    error
      (Custom
         ("Redefinition of class " ^ s_type_path cpath
        ^ ", please check using -v that the file is not referenced several \
           times"))
      (pos s);
  ...
  let herits =
    List.map
      (function
        ...
        | (HDynamic | HIntrinsic) as x -> x)
      herits
  in
  let is_component =
    !use_components && match clctx.path with "mx" :: _, _ -> true | _ -> false
  in
  let herits = if is_component then HIntrinsic :: herits else herits in
  Obj.set_field (Obj.repr s) 0
    (Obj.repr
       (if interf then EInterface (cpath, herits, e)
       else EClass (cpath, herits, e)));
  let rec loop flag = function
    | [] -> t_object ctx
    | HExtends cpath :: l ->
        if flag then
          error (Custom "Multiple inheritance is not allowed") (pos e);
        let cl = resolve_path ctx cpath (pos e) in
        if clctx.interface && not cl.interface then
          error (Custom "Interface cannot extends a class") (pos e);
        ignore (loop true l);
        cl
    | _ :: l -> loop flag l
  in
  clctx.super <- loop false herits;
  if clctx.super.interface && not clctx.interface then
    error (Custom "Cannot extends an interface") (pos e);
  let rec loop = function
    | [] -> []
  ...
  clctx.implements <-
    List.map
      (fun cpath ->
        ...
        if clctx.interface then
          error
            (Custom "Interface cannot implements another interface, use extends")
            (pos e);
        if not c.interface then
          error (Custom "Cannot implements a class") (pos e);
        c)
      (loop herits);
...
let type_file ctx req_path file el pos =
  ...
  let imports = { paths = Hashtbl.create 0; wildcards = [] } in
  let clerror t p =
    if pos = argv_pos then ()
    else if
      String.lowercase_ascii (s_type_path req_path)
      = String.lowercase_ascii (s_type_path t)
    then (
      ...
      if List.exists (( = ) f) a then error (Class_name_mistake req_path) p
      else error (Class_name_mistake t) pos
  in
  List.iter
    (fun ((s, p) as sign) ->
      match s with
      | EClass (t, hl, e) ->
          ...
          if !clctx <> None then
            error (Custom "Cannot declare several classes in same file") p;
          clctx :=
            Some
              (type_class ctx t hl e imports file false
                 (List.exists (( = ) HIntrinsic) hl)
                 sign)
      | EInterface (t, hl, e) ->
          if t <> req_path then clerror t (snd e);
          if !clctx <> None then
      ...
      | EImport (path, Some name) ->
          if Hashtbl.mem imports.paths name then
            error (Custom "Duplicate Import") p;
          Hashtbl.add imports.paths name
            { imp_path = (path, name); imp_pos = p; imp_used = false }
      | EImport (pk, None) ->
          imports.wildcards <-
            { wimp_path = pk; wimp_pos = p; wimp_used = false }
            :: imports.wildcards)
    el;
  if
    !warn_imports
    && ((not !use_components)
       || match !clctx with Some { path = "mx" :: _, _ } -> false | _ -> true)
  then
    add_finalizer ctx (fun () ->
        Hashtbl.iter
          (fun _ imp ->
            if not imp.imp_used then
              !Parser.warning "import not used" imp.imp_pos)
          imports.paths;
        List.iter
          (fun imp ->
            if not imp.wimp_used then
              !Parser.warning "import not used" imp.wimp_pos)
          imports.wildcards);
  !clctx
...
let load_file ctx file =
  ...
  let expr, comments =
    try Parser.parse (Lexing.from_channel ch) file
    with exc ->
      close_in ch;
      raise exc
  in
...
let rec load_class ctx path p =
  ...
  | [ param ], "Array" when param.[0] == '#' ->
      ...
      let rec map_type = function
        ...
        | Function (params, ret) ->
            Function (List.map map_type params, map_type ret)
        | t -> t
      in
      Hashtbl.iter
        (fun s f ->
      ...
      arr
  | _ -> (
      try Hashtbl.find ctx.classes path
      with Not_found -> (
        if String.lowercase_ascii (snd path) = "con" then
          error
            (Custom
               "CON is a special file under Windows and shouldn't be used as \
                class name")
            p;
        let file_name =
          match fst path with
        ...
        try
          ...
          | None ->
              error (Custom "Missing class definition")
                { pfile = file_name; pmin = 0; pmax = 0 }
          | Some c -> c
        with File_not_found _ -> error (Class_not_found path) p))
  ...
  Hashtbl.iter
    (fun _ clctx ->
      ...
      let rec loopeq variance t1 t2 =
        ...
        | Dyn, _ -> true
        | Class cl1, Class cl2 -> (
            if cl1.path = cl2.path then true
            else
              let t =
                try tcommon ctx (Class cl1) (Class cl2) null_pos with _ -> Dyn
              in
              match t with
        ...
        | Function (a1, r1), Function (a2, r2)
          when List.length a1 = List.length a2 ->
            List.for_all2 (loopeq true) a1 a2 && loopeq false r1 r2
        | _, _ -> false
      in
          ...
          (fun _ f ->
            if f.f_static = IsMember then
              ...
              | None ->
                  error
                    (Custom
                       ("Missing field " ^ f.f_name ^ " required by "
                      ^ s_type_path i.path))
                    { pfile = clctx.file; pmin = 0; pmax = 0 }
              | Some f2 ->
                  if f2.f_public = IsPrivate then
                    error
                      (Custom
                         ("Field " ^ f.f_name
                        ^ " is declared in an interface and should be public"))
                      f2.f_pos;
                  unify f2.f_type f.f_type f2.f_pos;
                  if not (loopeq true f.f_type f2.f_type) then
                    error
                      (Custom
                         ("Field " ^ f.f_name
                        ^ " type is different from the one defined in "
                        ^ s_type_path i.path))
                      f2.f_pos)
          i.fields
      in
